...
function SearchPage(props) {
    ...
    const showProducts = () => {
        let result = products.slice(pagination * 20, pagination * 20 + 20).map((item, index) => {
            return <Product id={item._id} name={item.name} url={item.url} price={item.price} key={index} />;
        });
        return result;
    };
